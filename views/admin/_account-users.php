        ...
        <div class="col-12">
            ...
            <div class="row">
                <div class="col-12 mt-5 mb-3">
                    <div class="meta">
                        <h2 class="h1"><?php _e('Beneficjenci', 'pern') ?></h2>
                        <?php if (!empty($posts)) : ?>
                ...
                <?php if (!empty($posts)) : ?>
                    <tbody>
                        <?php foreach ($posts as $post) : ?>
                            <?php
                            $status = $post['org_status'] === 'true' ? __('Zaakceptowany', 'pern') : ($post['org_status'] === 'false' ? __('Odrzucony', 'pern') : __('W trakcie', 'pern')); ?>
                            
                            <tr>
                                <td><input type="checkbox" class="export-checkbox"></input></td>
                                <td><a href="<?= $post['post_link']; ?>"><?= $post['project_name']; ?></a></td>
                                <td><?= $post['project_email']; ?></td>
                                <td><?= $post['post_date']; ?></td>
                                <td><?= $post['org_type']['label']; ?></td>
                                <td><?= $post['krs']; ?></td>
                                <td><span class="status <?= $post['org_status']; ?>"><?= $status; ?></span></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                <?php else : ?>
                    <tbody>
                        <tr>
                            <td><?php _e('Brak aktywnych użytkowników', 'pern') ?></td>
                        </tr>
                    </tbody>
                <?php endif; ?>
            </table>
